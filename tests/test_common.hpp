...
static void minigdbstubUsrProcessBreakpoint(int type, size_t addr, void *usrData)
{
    ...
    if (type & MGDB_HARD_BREAKPOINT)
        brkObj->bits.hardBreak = 1;
    if (type & MGDB_SOFT_BREAKPOINT)
        brkObj->bits.softBreak = 1;
    if (type & MGDB_CLEAR_BREAKPOINT)
        brkObj->bits.isClear = 1;
    if (type & MGDB_SET_BREAKPOINT)
        brkObj->bits.isSet = 1;
    return;
}
